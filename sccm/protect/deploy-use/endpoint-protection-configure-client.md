---
title: Endpoint Protection-Client configureren | Microsoft-documenten
description: "Informatie over het configureren van aangepaste clientinstellingen voor Endpoint Protection die kunnen worden geïmplementeerd naar verzamelingen van computers in uw hiërarchie."
ms.custom: na
ms.date: 02/14/2017
...
Deze procedure configureert u aangepaste clientinstellingen voor Endpoint Protection die kan worden geïmplementeerd op verzamelingen computers in uw hiërarchie.

> [!IMPORTANT]
>  De standaardinstellingen van de Endpoint Protection-client alleen configureren als u zeker weet dat u ze toepassen op alle computers in uw hiërarchie wilt.
...
5.  Configureer de clientinstellingen voor Endpoint Protection die u nodig hebt. Voor een volledige lijst van Endpoint Protection-client-instellingen die u kunt configureren, Zie de sectie Endpoint Protection in [over clientinstellingen in System Center Configuration Manager](../../core/clients/deploy/about-client-settings.md).
...
Clientcomputers zullen worden geconfigureerd met deze instellingen wanneer ze de volgende keer het clientbeleid downloaden. Als u wilt ophalen van beleid voor één client hebt gestart, Zie het Ophaalbeleid initiëren voor een Configuration Manager-Client-sectie in [in System Center Configuration Manager-clients beheren](../../core/clients/manage/manage-clients.md).
...
U kunt de Endpoint Protection-client installeren op een computer die u wilt gebruiken als bron voor schijfinstallatiekopieën voor besturingssysteemimplementatie voor Configuration Manager. Deze computer wordt doorgaans de referentiecomputer genoemd. Nadat u de installatiekopie van het besturingssysteem hebt gemaakt, klikt u vervolgens kunt u de implementatie van de Configuration Manager-besturingssysteem om de installatiekopie die u softwarepakketten bevatten kunt, waaronder Endpoint Protection op uw clientcomputers.

Gebruik de procedures in dit onderwerp om te installeren en configureren van de Endpoint Protection-client op een referentiecomputer
...
De volgende lijst bevat de vereiste onderdelen voor het installeren van de Endpoint Protection-clientsoftware op een referentiecomputer.
...
-   Om te controleren of de Endpoint Protection client wordt geïmplementeerd met de configuratie die is in uw organisatie vereist, maakt u een antimalwarebeleid en exporteert u dat beleid. U kunt vervolgens opgeven welk antimalwarebeleid moet worden gebruikt wanneer u de Endpoint Protection-client handmatig installeert. Zie voor meer informatie [maken en implementeren van antimalware-beleidsregels voor Endpoint Protection in System Center Configuration Manager](endpoint-antimalware-policies.md).

   > [!NOTE]
   >  Het beleid dat is ingesteld als **Standaard antimalwarebeleid client** kan niet worden geëxporteerd.

-   Als u de Endpoint Protection-client installeert met de meest recente definities wilt, u moet deze downloaden vanuit het [Microsoft Malware Protection Center](http://go.microsoft.com/fwlink/?LinkID=200965).
...
4.  Als u het meest recente pakket met updatedefinities hebt gedownload, kopieert u dit pakket naar de clientcomputer en dubbelklikt u op het pakket om het te installeren.
   ...
   > [!NOTE]
   >  Nadat de installatie van de Endpoint Protection-client is voltooid, voert de client automatisch een definitie-update uit. Als er een update beschikbaar is, hoeft u het pakket met de meest recente definitie-update niet handmatig te installeren.
...
1.  Open op de referentiecomputer **System Center Endpoint Protection** via Windows melding zijn.

2.  Op de **Start** tabblad van het **System Center Endpoint Protection** dialoogvenster Controleer **real-timebeveiliging** is ingesteld op **op**.
...
Nadat u verifieert of de Endpoint Protection-client correct is geïnstalleerd op de referentiecomputer, kunt u de computer voorbereiden voor imaging. Voer de volgende stappen uit om de Endpoint Protection-client voorbereiden voor imaging.
