---
title: Instellen van uw testomgeving System Center Configuration Manager | Microsoft-documenten
description: Instellen van een testlab voor het evalueren van Configuration Manager met gesimuleerde concrete activiteiten.
ms.custom: na
ms.date: 10/06/2016
...
---
# <a name="set-up-your-system-center-configuration-manager-lab"></a>Instellen van uw testomgeving System Center Configuration Manager

*Van toepassing op: System Center Configuration Manager (huidige vertakking)*

De instructies in dit onderwerp te volgen, kunt u voor het instellen van een testlab voor het evalueren van Configuration Manager met gesimuleerde concrete activiteiten.  

##  <a name="BKMK_LabCore"></a> Kernonderdelen  
 Instellen van uw omgeving voor System Center Configuration Manager vereist een aantal kernonderdelen ter ondersteuning van de installatie van Configuration Manager.    

-   **De testomgeving maakt gebruik van Windows Server 2012 R2**, waarnaar we System Center Configuration Manager wordt geïnstalleerd.  

     U kunt een evaluatieversie van Windows Server 2012 R2 van downloaden de [TechNet-Evaluatiecentrum](https://www.microsoft.com/evalcenter/evaluate-windows-server-2012).  

     U kunt wijzigen of verbeterde beveiliging van Internet Explorer voor eenvoudiger bepaalde de downloads waarnaar wordt verwezen tijdens de duur van deze uitschakelen. Controleer [Internet Explorer: Verbeterde beveiliging](https://technet.microsoft.com/en-us/library/dd883248\(v=ws.10\).aspx) voor meer informatie.  

-   **De testomgeving gebruikt SQL Server 2012 SP2** voor de sitedatabase.  

     U kunt een evaluatieversie van SQL Server 2012 van downloaden de [Microsoft Download Center](https://www.microsoft.com/en-us/download/details.aspx?id=29066).  
    ...
    -   Een exclusieve **SQL Server-exemplaar** is vereist.  

    -   Beperken niet de **adresseerbare systeemgeheugen** voor SQL Server.  

    -   Configureer de **SQL Server-serviceaccount** uitvoeren met behulp van de **lokale domeingebruiker** account.  

    -   U moet installeren **SQL Server reporting services**.  

    -   **Intersite-communicatie** gebruik van de SQL Server Service Broker op standaardpoort TCP 4022.  

    -   **Intrasitecommunicatie** tussen de SQL Server database engine en selecteer Configuration Manager-sitesysteem rollen gebruiken standaardpoort TCP 1433.  

-   **De domeincontroller maakt gebruik van Windows Server 2008 R2** met Active Directory Domain Services. De domeincontroller ook fungeert als host voor de DHCP- en de DNS-servers voor gebruik met een volledig gekwalificeerde domeinnaam.  
...
-   **Hyper-V wordt gebruikt met een paar virtuele machines** om te verifiëren dat de stappen in deze oefeningen management werken zoals verwacht. Minimaal drie virtuele machines wordt aanbevolen, Windows 7 (of hoger) geïnstalleerd.  

     Raadpleeg voor meer informatie dit [overzicht van Hyper-V](https://technet.microsoft.com/en-us/library/hh831531.aspx).  

-   **Beheerdersmachtigingen** vereist is voor al deze onderdelen.  

    -   Configuration Manager vereist een beheerder met lokale machtigingen vanuit de Windows Server-omgeving  

    -   Active Directory moet een beheerder met machtigingen om het schema te wijzigen  

    -   Virtuele machines vereisen lokale machtigingen op de virtuele machines zelf  

Hoewel dit niet vereist voor dit lab, kunt u bekijken [ondersteunde configuraties voor System Center Configuration Manager](../../core/plan-design/configs/supported-configurations.md) voor meer informatie over vereisten voor de implementatie van System Center Configuration Manager. Raadpleeg de documentatie voor softwareversies dan die hier wordt verwezen.  

Als u al deze onderdelen hebt geïnstalleerd, zijn er extra stappen die u ondernemen moet om uw Windows-omgeving voor Configuratiebeheer configureren:  
...
De volgende stappen nodig voor het inschakelen van Configuration Manager-clients naar query Active Directory Domain Services om sitebronnen te vinden worden over de volgende procedures weergegeven.  

###  <a name="BKMK_CreateSysMgmtLab"></a> De container voor systeembeheer maken  
 Configuration Manager maakt niet automatisch de vereiste Systeembeheer-container in Active Directory Domain Services wanneer het schema wordt uitgebreid. Daarom maakt u deze voor uw testomgeving. Voor deze stap moet u [ADSI Bewerken installeren](https://technet.microsoft.com/en-us/library/cc773354\(WS.10\).aspx#BKMK_InstallingADSIEdit).  
...
2.  Vouw **domein&lt;computer volledig gekwalificeerde domeinnaam\>**, vouw **< DN-naam\>**, met de rechtermuisknop op **CN = systeem**, klikt u op **New**, en klik vervolgens op **Object**.  
...
1.  Vouw in het consolevenster het **serverdomein site**, vouw **DC =&lt;server DN-naam\>**, en vouw vervolgens **CN = systeem**. Klik met de rechtermuisknop op **CN=Systeembeheer**en klik dan op **Eigenschappen**.  
...
5.  Klik op **OK** om de **ADSI Bewerken** -console te sluiten en de procedure te voltooien.  
     ...
     Raadpleeg voor meer inzicht in deze procedure [het Active Directory-schema uitbreidt voor System Center Configuration Manager](../../core/plan-design/network/extend-the-active-directory-schema.md)  

###  <a name="BKMK_ExtADSchLab"></a> Het Active Directory-schema met behulp van extadsch.exe uitbreiden  
 U wordt het Active Directory-schema voor deze lab uitbreiden als u dit kunt u alle Configuration Manager-functies en functionaliteiten gebruiken met zo min mogelijk administratieve overhead. Het uitbreiden van het Active Directory-schema is een configuratie voor het hele forest, die één keer per forest wordt uitgevoerd. Permanente uitbreiding van het schema wijzigt de set klassen en kenmerken in de basisconfiguratie van Active Directory. Deze actie kan niet ongedaan worden gemaakt. Het schema uitbreidt, kunt Configuration Manager voor toegang tot de onderdelen waarmee het meest effectief binnen uw testomgeving.  
...
-   [Procedures: Upgrade van een ASP.NET-webtoepassing voor ASP.NET 4](https://technet.microsoft.com/en-us/library/dd483478\(VS.100\).aspx)  
...
-   [CLR binnenuit - In-Process Side-by-Side](https://msdn.microsoft.com/en-us/magazine/ee819091.aspx)  
...
Internet Information Services (IIS) is een flexibele, schaalbare webserver die kan worden gebruikt voor het hosten van alles op het web. Door Configuration Manager wordt gebruikt voor een aantal sitesysteemrollen. Raadpleeg voor meer informatie over IIS [Websites voor systeemservers van sites in System Center Configuration Manager](../../core/plan-design/network/websites-for-site-system-servers.md).  
...
Maakt u een [bepalen wanneer een primaire site gebruiken](../../core/plan-design/hierarchy/design-a-hierarchy-of-sites.md#BKMK_ChoosePriimary) rechtstreeks beheren van clients. Zo kunt u uw testomgeving ter ondersteuning van beheer voor [Site system schaal](/sccm/core/plan-design/configs/size-and-scale-numbers) mogelijke apparaten.  
...
Voordat u de installatie, start de [vereistencontrole](/sccm/core/servers/deploy/install/prerequisite-checker) op de server met Windows Server 2012 om te bevestigen dat alle instellingen correct zijn ingeschakeld.  
...
3.  Volg de installatieprocedure beschreven in [installeren van een site met de Setup Wizard van System Center Configuration Manager](/sccm/core/servers/deploy/install/use-the-setup-wizard-to-install-sites). In deze procedure geeft u de volgende invoer op:  
    ...
    |Stap 11: **Installatie van de primaire Site**|Selecteer **De primaire site installeren als een zelfstandige site**en klik vervolgens op **Volgende**.|  
...
###  <a name="BKMK_EnablePubLab"></a>Inschakelen van publicatie voor de Configuration Manager-site  
