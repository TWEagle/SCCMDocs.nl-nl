---
title: Cloud Management Gateway instellen | Microsoft Docs
description: 
author: robstackmsft
ms.author: robstack
manager: angrobe
...
7.  Voltooi de Wizard Certificaat exporteren met behulp van de standaardindeling van het certificaat. Controleer de naam en locatie van het basiscertificaat dat u maakt. U hebt deze voor het configureren van cloud-management-gateway in een [later stap](#step-4-set-up-cloud-management-gateway).
...
6. Controleer de instellingen en kies **volgende**. Configuration Manager start instellen van de service. Nadat u de wizard te sluiten die wordt uitgevoerd tussen 5 tot 15 minuten voor het inrichten van de service volledig in Azure. Controleer de **Status** kolom voor de nieuwe installatie management cloudgateway om te bepalen wanneer de service is klaar.
...
De laatste stap bij het instellen van de cloud management gateway is voor het configureren van de sitesysteemrollen voor het accepteren van verkeer van de cloud management gateway. Alleen de management point- and -software-update-punt functies worden ondersteund voor cloud management gateway. U moet elke rol afzonderlijk configureren.
...
Met de locatie van de cloud management gateway-service op de client is geconfigureerd, kunt u automatisch bepalen of deze zich op het intranet of Internet. Als de client kan verbinding maken met de domeincontroller of het lokale beheerpunt, wordt dit gebruikt voor communicatie met Configuration Manager, anders wordt deze beschouwd wordt op het Internet en gebruik de locatie van de cloud management gateway-service om te communiceren.
