---
title: Asset Intelligence Inleiding | Microsoft-documenten
description: Kennismaking met Asset Intelligence in System Center Configuration Manager.
ms.custom: na
ms.date: 2/22/2017
...
> [!NOTE]  
>  De hardwarevereisten weergegeven in de Configuration Manager-console worden opgehaald uit de Asset Intelligence-catalogus en niet op basis van de titel informatie over geïnventariseerde software van System Center 2012 Configuration Manager-clients. Informatie over hardwarevereisten wordt niet bijgewerkt tijdens de synchronisatie met System Center Online. U kunt door de gebruiker gedefinieerde hardwarevereisten maken voor geïnventariseerde software die geen bijbehorende hardwarevereisten heeft.  

 Standaard wordt de volgende informatie weergegeven voor elke vermelde hardwarevereiste:  

-   **Softwaretitel**: Hiermee geeft u de softwaretitel die zijn gekoppeld aan de hardwarevereiste.  

-   **Minimale CPU (MHz)**: Hiermee geeft u de minimale processorsnelheid, in megahertz (MHz) vereist voor de softwaretitel.  

-   **Minimale hoeveelheid RAM (KB)**: Hiermee geeft u de minimale hoeveelheid RAM, in kilobyte (KB), vereist voor de softwaretitel.  

-   **Minimale schijfruimte (KB)**: Hiermee geeft u de minimale vrije schijfruimte, in KB, door de softwaretitel vereist.  

-   **Minimale grootte (KB)**: Geeft de minimale hardeschijfomvang in KB, vereist voor de softwaretitel.  
...
> [!WARNING]  
>  De clientagent voor hardware-inventarisatie verzamelt inventaris op basis van de Asset Intelligence-rapportageklassen voor hardware-inventarisatie die u inschakelt. Zie voor meer informatie over het inschakelen van de rapportage-klassen, [Asset Intelligence configureren in System Center Configuration Manager](../../../../core/clients/manage/asset-intelligence/configuring-asset-intelligence.md).  

 Standaard wordt de volgende informatie weergegeven voor elke geïnventariseerde softwaretitel:  

-   **Naam**: Hiermee geeft u de naam van de titel van de geïnventariseerde software.  

-   **Leverancier**: Hiermee geeft u de naam van de leverancier van de titel van de geïnventariseerde software ontwikkeld.  

-   **Versie**: Hiermee geeft u de productversie van de titel van de geïnventariseerde software.  

-   **Categorie**: Hiermee geeft u de softwarecategorie dat momenteel is toegewezen aan de titel van de geïnventariseerde software.  

-   **Familie**: Hiermee geeft u de softwarefamilie dat momenteel is toegewezen aan de titel van de geïnventariseerde software.  

-   **Label** [**1**, **2**, and **3**]: Hiermee geeft u de aangepaste labels die gekoppeld aan de softwaretitel zijn. Aan geïnventariseerde softwaretitels kunnen maximaal drie aangepaste labels worden gekoppeld.  

-   **Aantal**: Hiermee geeft u het nummer van Configuration Manager-clients waarop de softwaretitel zijn geïnventariseerd.  

-   **Status**: Hiermee geeft u de validatiestatus voor de titel van de geïnventariseerde software.  
...
##  <a name="BKMK_AssetIntelligenceHomePage"></a> Asset Intelligence-startpagina  
 De **Asset Intelligence** knooppunt in de **activa en naleving** werkruimte is de startpagina voor Asset Intelligence in Configuration Manager. Op de startpagina **Asset Intelligence** wordt een overzichtsdashboard voor Asset Intelligence-catalogusgegevens weergegeven.  
...
-   **Status van de Software geïnventariseerde**: Geeft het aantal en percentage van de geïnventariseerde software, softwarecategorieën en softwarefamilies die door Microsoft, geïdentificeerd door een beheerder, in afwachting van online identificatie of niet geïdentificeerd en niet in afwachting worden geïdentificeerd. De informatie in tabelindeling bevat het aantal voor elke softwaretoepassing, terwijl in de grafiek het percentage voor elke softwaretoepassing wordt weergegeven.  
...
###  <a name="BKMK_SoftwareIdTagReports"></a> Asset Intelligence-rapporten voor de identificatie van softwaretags  
 Asset Intelligence-rapporten voor de identificatie van softwaretags bevatten informatie over software met een softwaretag die in overeenstemming is met ISO/IEC 19770-2. De softwaretags bieden gezaghebbende informatie die wordt gebruikt voor het identificeren van geïnstalleerde software. Wanneer u de rapportageklasse hardware-inventarisatie inschakelt, wordt in Configuration Manager informatie over de software met software-ID-tags verzamelt. De volgende rapporten bevatten informatie over de software:  

-   **Software 14A - zoeken naar software waarbij een softwaretag is geactiveerd**: Dit rapport bevat het aantal geïnstalleerde software met een software-id-tag is geactiveerd.  

-   **Software 14B - Computers met specifieke software-id-tag is geactiveerd geïnstalleerd**: Dit rapport geeft alle computers waarop software is geïnstalleerd met een specifieke software-id-tag is geactiveerd.  

-   **Software 14C - geïnstalleerde software software-identificatielabel op een specifieke computer**: Dit rapport geeft alle geïnstalleerde software met een specifieke software-id-tag is geactiveerd op een specifieke computer.  
 ...
 **Afhankelijkheden van clienthardware-inventaris**  
 Asset Intelligence geïnstalleerd software-rapporten zijn gebaseerd op gegevens die worden verzameld van Configuration Manager-clients door het uitbreiden van hardware-inventaris om aan te melden van Asset Intelligence inschakelen. Asset Intelligence-rapporten geven vanwege deze afhankelijkheid van hardware-inventarisatie, alleen de gegevens van Configuration Manager-clients die door processen voor hardware-inventaris met succes voltooid met de vereiste Asset Intelligence WMI-rapportklassen ingeschakeld. Bovendien omdat Configuration Manager-clients processen voor hardware-inventaris volgens een schema gedefinieerd door de gebruiker met beheerdersrechten voeren, optreden een vertraging gegevensrapportage die van invloed is op de nauwkeurigheid van Asset Intelligence-rapporten. Een geïnventariseerde gelicentieerde softwaretitel kan bijvoorbeeld zijn verwijderd nadat op de client een geslaagde hardware-inventarisatiecyclus is uitgevoerd. Echter, de softwaretitel weergegeven zoals in Asset Intelligence-rapporten geïnstalleerd totdat de client volgende geplande hardware-inventarisatie cyclus.  

 **Afhankelijkheden van softwarepakketten**  
 Omdat Asset Intelligence-rapporten zijn gebaseerd op gegevens van de geïnstalleerde software-titel die is verzameld door standaard Configuration Manager client hardware-inventaris processen, kunnen sommige gegevens van de titel software niet correct worden verkregen. Software-installaties die niet voldoen aan standaardinstallatieprocessen of software-installaties die zijn gewijzigd vóór de installatie, kunnen bijvoorbeeld onnauwkeurigheden in Asset Intelligence-rapporten veroorzaken.  
 ...
 **Beperkingen voor het aantal geïnstalleerde softwaretitels**  
 Configuration Manager-Clients moet hardware-inventarisatie cycli voor de Asset Intelligence-rapporten goed rapporteert het aantal geïnstalleerde softwaretitels voltooien. Daarnaast kan er na een geslaagde rapportagecyclus voor de hardware-inventaris een vertraging tussen het installeren of verwijderen van een gelicentieerde softwaretitel optreden die pas in Asset Intelligence-rapporten wordt weergegeven na de volgende geplande hardware-inventarisatie.  

 **Beperkingen voor licentieafstemming**  
 Het afstemmen van het aantal titels geïnstalleerde software op de hoeveelheid aangekochte softwarelicenties wordt berekend met behulp van een vergelijking van het licentieaantal is opgegeven door de beheerder en de hoeveelheid geïnstalleerd softwaretitels die worden verzameld van Configuration Manager client hardware inventarissen op basis van de door de beheerder ingestelde planning. Deze vergelijking weerspiegelt niet een definitieve Microsoft-conclusie van de licentieposities. De werkelijke licentiepositie is afhankelijk van de specifieke softwaretitellicentie en verleende gebruiksrechten door de licentievoorwaarden.  
...
|**Niet-gecategoriseerd**|Catalogusitem is niet gedefinieerd door System Center Online-onderzoekers, het item is niet naar System Center Online verzonden voor categorisatie en de beheerder heeft geen door de gebruiker gedefinieerde categoriewaarde toegewezen.|Geen.|Vraag categorisatie aan of pas lokale catalogusgegevens aan.<br /><br /> Zie voor meer informatie over de aanvragende categorisatie [bewerkingen voor Asset Intelligence in System Center Configuration Manager](../../../../core/clients/manage/asset-intelligence/operations-for-asset-intelligence.md).<br /><br /> Zie voor meer informatie over het wijzigen van de categorie voor de softwaretitel [bewerkingen voor Asset Intelligence in System Center Configuration Manager](../../../../core/clients/manage/asset-intelligence/operations-for-asset-intelligence.md).|  
...
>  Nadat een categorisatieconflict is opgelost, wordt het item niet meer gevalideerd als conflicterend tenzij in latere categorisatie-updates nieuwe informatie over het item wordt gepresenteerd.  
 ...
 Zie voor meer voorbeelden van wanneer een validatiestatus status van één naar een andere verandert mogelijk [voorbeeld validatie statusovergangen voor Asset Intelligence in System Center Configuration Manager](../../../../core/clients/manage/asset-intelligence/example-validation-state-transitions-for-asset-intelligence.md).  
