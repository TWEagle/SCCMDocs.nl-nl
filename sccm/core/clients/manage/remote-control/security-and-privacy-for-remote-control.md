---
title: Beheer op afstand beveiliging privacy | Microsoft-documenten
description: Beveiliging en privacy-informatie voor beheer op afstand in System Center Configuration Manager ophalen.
ms.custom: na
ms.date: 04/23/2017
...
|----------------------------|----------------------|  
|Als u verbinding wilt maken met een externe computer, moet u stoppen als NTLM-verificatie wordt gebruikt in plaats van Kerberos-verificatie.|Als Configuration Manager detecteert dat de sessie voor beheer op afstand wordt geverifieerd via NTLM in plaats van Kerberos, ziet u een opdrachtprompt met een waarschuwing dat de identiteit van de externe computer kan niet worden geverifieerd. Ga niet verder met de sessie voor beheer op afstand. NTLM-verificatie is een zwakker verificatieprotocol dan Kerberos en is kwetsbaar voor opnieuw afspelen en imitatie.|  
...
|Vergrendel het toetsenbord en de muis tijdens een sessie voor beheer op afstand.|Als Configuration Manager detecteert dat de beheer op afstand verbinding is verbroken, wordt het toetsenbord en muis door Configuration Manager vergrendeld, zodat een gebruiker kan geen controle van de sessie voor beheer op afstand openen. Deze detectie vindt mogelijk echter niet direct plaats en vindt niet plaats als de service voor beheer op afstand is beëindigd.<br /><br /> Selecteer de actie **Extern toetsenbord en muis vergrendelen** in het venster **Afstandbediening ConfigMgr** .|  
...
-   Beschouw controleberichten voor beheer op afstand niet als betrouwbaar.  
     ...
     Controleberichten worden niet verzonden als u de binaire bestanden voor beheer op afstand kopieert in plaats van de Configuration Manager-console installeren en voert u beheer op afstand vanaf de opdrachtregel.  

##  <a name="BKMK_Privacy_HardwareInventory"></a> Privacy-informatie voor beheer op afstand  
 Beheer op afstand kunt u actieve sessies op Configuration Manager-clientcomputers weergeven en gegevens die zijn opgeslagen op deze computers mogelijk te bekijken. Beheer op afstand is standaard niet ingeschakeld.  
 ...
 Standaard verleent Configuration Manager de lokale groep Administrators groepsmachtigingen voor beheer op afstand.  
