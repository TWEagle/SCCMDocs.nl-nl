---
title: Controleren van replicatie | Microsoft-documenten
description: Informatie over het bewaken van de infrastructuur en bewerkingen in Configuration Manager met behulp van de werkruimte voor bewaking in de console.
ms.custom: na
ms.date: 10/06/2016
...
Gebruiken voor het controleren van de infrastructuur en bewerkingen in System Center Configuration Manager, de **bewaking** werkruimte in de Configuration Manager-console.  
...
>  De uitzondering op deze locatie is Migratie, die direct gecontroleerd wordt in knooppunt **Migratie** in de werkruimte **Beheer** . Voor meer informatie, zie [Bewerkingen voor de migratie naar System Center Configuration Manager](../../../core/migration/operations-for-migration.md).  
 ...
 U kunt naast het gebruik van de Configuration Manager-console voor bewaking, gebruikt u de Configuration Manager-rapporten of Configuration Manager-logboekbestanden voor Configuration Manager-onderdelen weergeven. Zie [Rapportage in System Center Configuration Manager](../../../core/servers/manage/reporting.md) voor meer informatie over rapporten. Zie [Logbestanden in System Center Configuration Manager](../../../core/plan-design/hierarchy/log-files.md) voor meer informatie over logbestanden.  
...
##  <a name="BKMK_MonintorMgmtTasks"></a> Algemene beheertaken voor Configuration Manager bewaken  
 Configuration Manger voorziet ingevouwde bewaking in de Configuration Manager-console. U kunt vele taken bewaken, inclusief deze met betrekking tot software-updates, energiebeheer en de implementatie van inhoud via uw hiërarchie.  

 Gebruik de volgende informatie als u algemene taken voor Configuration Manager controleren:  
...
Configuration Manager beschikt over verschillende methoden om de status en bewerkingen van uw hiërarchie te controleren. U kunt de sitesysteemstatus door de hiërarchie verifiëren, intersitereplicatie van een sitehiërarchie of geografische weergave controleren, replicatiekoppelingen tussen sites voor databasereplicatie controleren en het Replication Link Analyser-hulpprogramma gebruiken voor het oplossen van replicatieproblemen.  
...
-   **Hiërarchiediagram**: In deze weergave toont uw hiërarchie als een topologiekaart die vereenvoudigde topologiekaart enkel belangrijke informatie toont.  

-   **Geografische weergave**: Deze weergave toont uw sites op een geografische kaart op die sitelocaties die u configureert.  
...
Omdat de site- en intersite status repliceren als sitegegevens en niet als globale gegevens koppelen als u uw Configuration Manager-console verbinding met een onderliggende primaire site maakt, kunt u de status van de site of de koppelingsstatus voor andere primaire sites of hun onderliggende secundaire sites niet weergeven. Bijvoorbeeld in een multiprimaire sitehiërarchie, wanneer de Configuration Manager-console verbinding met een primaire site maakt, ziet u de status van onderliggende secundaire sites, de primaire site en de centrale beheersite, maar u kunt de status van andere knooppunten van de hiërarchie onder de centrale beheersite niet zien.  

 Gebruik de opdracht **Instellingen configureren** om te controleren hoe de weergave van de sitehiërarchie uitgevoerd is. Configuraties die u kunt de **sitehiërarchie** knooppunt dat u maakt bij de Configuration Manager-console is verbonden met een site worden gerepliceerd naar andere sites.  
...
#### <a name="hierarchy-diagram"></a>Diagram van hiërarchie  
 ...
 Daarnaast kunt u de muisaanwijzer op een site of replicatiekoppeling tussen sites om de statuswaarde op hoog niveau voor dat object weer te onderbreken. Omdat de replicatiestatus van de link repliceert niet globaal, in een hiërarchie met meerdere primaire sites, moet u uw Configuration Manager-console verbinden met de centrale beheersite om de details van replicatie tussen alle sites weer te geven.  

 De volgende opties wijzigen het hiërarchiediagram:  

-   **Groepen**: Hier kunt u het aantal primaire sites en secundaire sites die een wijziging activeren in de weergave van het hiërarchiediagram dat de sites in een enkel object combineert. Als sites worden gecombineerd tot een enkel object, ziet u het totale aantal sites en een hoge rollup van statusberichten en sitestatus. Groepsconfiguraties hebben geen invloed op de geografische weergave.  
...
> [!NOTE]  
>  Voor het gebruik van de geografische weergave wordt de computer waarop uw Configuration Manager-console verbinding moet Internet Explorer hebben geïnstalleerd en kunnen toegang tot Bing-kaarten met HTTP-protocol.  

De volgende optie wijzigt de geografische weergave.  

-   **Sitelocatie**: U kunt een geografische locatie voor elke site opgeven. U kunt de locatie opgeven als een straatadres, een plaatsnaam zoals de naam van een stad of aan de hand van coördinaten voor de breedte- en lengtegraad. U geeft **N 47 40 26.3572 W 122 7 17.4432** op als de sitelocatie, om bijvoorbeeld de breedte- en lengtegraad van Redmond Washington te gebruiken. De symbolen voor de graden, minuten of seconden van lengte of breedte moet u niet opgeven. Configuration Manager gebruikt Bing-kaarten om weer te geven van de locatie op de geografische weergave. Dit biedt u de optie om uw hiërarchie weer te geven met betrekking tot een geografische locatie, die inzicht kan geven in regionale problemen en die mogelijk specifieke sites of intersitereplicatie kunnen betreffen.  
...
**Initialisatiedetails**    
 ...
 U kunt deze informatie bovendien gebruiken om te identificeren wanneer een site zich mogelijk in een interoperabiliteitsmodus bevindt. Interoperabiliteitsmodus treedt op wanneer de onderliggende site niet dezelfde versie van Configuration Manager wordt uitgevoerd als de bovenliggende site.  
...
###  <a name="BKMK_RLA"></a> Over de Replication Link Analyzer  
 Configuration Manager bevat **Replication Link Analyzer** die u gebruikt om te analyseren en te herstellen van problemen met replicatie. U kunt Replication Link Analyzer gebruiken om fouten in de replicatielink te herstellen wanneer replicatie gefaald heeft en wanneer replicatie stopt met werken maar nog niet gemeld werd als gefaald. Replication Link Analyzer kan worden gebruikt voor het oplossen van replicatieproblemen tussen de volgende computers in de Configuration Manager-hiërarchie (de richting van de Replicatiefout doet niet terzake):  

-   Tussen een siteserver en de sitedatabaseserver.  

-   Tussen een sitedabaseserver van site en een andere sites sitedatabasecomputer (intersite-replicatie).  

U kunt Replication Link Analyzer uitvoeren in de Configuration Manager-console of aan een opdrachtprompt:  

-   Uitvoeren in de Configuration Manager-console: In de **bewaking** werkruimte, klikt u op de **databasereplicatie** knooppunt, selecteer de replicatielink die u analyseren wilt, en klik vervolgens in de **databasereplicatie** groep op de **Start** tabblad **Replication Link Analyzer**.  

-   Als u wilt uitvoeren met een opdrachtprompt, typ de volgende opdracht: **%path%\Microsoft configuratie Manager\AdminConsole\bin\Microsoft.ConfigurationManager.ReplicationLinkAnalyzer.Wizard.exe &lt;bronsiteserver FQDN\> &lt;doelsiteserver FQDN-naam\>**  
...
-   Met de release van System Center Configuration Manager versie 1511 genereert de replication link analyzer fouten voor SQL Server Service Broker-certificaat voor primaire sites die van System Center 2012 Configuration Manager bijgewerkt. Dit komt door wijzigingen in de namen van de certificaten die zijn geïntroduceerd in versie 1511 waarvoor de Replication Link Analyzer nog niet zijn bijgewerkt. Deze fouten kunnen worden genegeerd.  

###  <a name="BKMK_ProcsforMonitoringReplication"></a> Procedures voor het bewaken van databasereplicatie  

##### <a name="to-monitor-high-level-site-to-site-database-replication-status"></a>Voor het bewaken van de replicatiestatus van de site-naar-site-database op hoog niveau    
