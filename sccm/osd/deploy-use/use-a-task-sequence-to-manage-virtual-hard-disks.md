---
title: Een takenreeks gebruiken voor het beheren van virtuele harde schijven | Microsoft-documenten
...
In System Center Configuration Manager kunt u virtuele harde schijven (VHD's) beheren en de schijven die u in uw datacenter vanuit de Configuration Manager-console maakt te integreren. U kunt specifiek, maken en een VHD wijzigen, toepassingen en software-updates toevoegen aan de VHD en de VHD publiceren naar System Center Virtual Machine Manager (VMM) vanuit de Configuration Manager-console.  
...
-   Virtualisatie moet zijn ingeschakeld in het BIOS en Hyper-V moet zijn geïnstalleerd op de computer van waaruit u de Configuration Manager-console voor het beheren van VHD's. Installeer als aanbevolen werkwijze ook de Hyper-V hulpprogramma's voor het beheer om uw virtuele harde schijven te testen en problemen op te lossen. De Hyper-V hulpprogramma's voor het beheer moeten bijvoorbeeld zijn geïnstalleerd voor het controleren van de smsts.log  om de voortgang van de takenreeks in Hyper-V op te volgen. Zie [Installatievereisten voor Hyper-V](http://technet.microsoft.com/library/cc731898.aspx)voor meer informatie over de vereisten voor Hyper-V.  

    > [!IMPORTANT]  
    >  Het proces om een VHD te maken verbruikt tijd en geheugen van de processor. Daarom is het aanbevolen dat u de VHD's beheert vanuit een Configuration Manager-console die niet is geïnstalleerd op de siteserver.  
...
-   Installeer de System Center Virtual Machine Manager (VMM)-console van waaruit u de VHD naar VMM uploadt. U kunt de VMM-console installeren op een afzonderlijke computer van waaruit u uw VHD's beheert. Dit betekent dat installatie van de Hyper-V niet vereist is om de VHD naar VMM te importeren.  
    ...
    > [!NOTE]  
    >  Als u de VMM-console installeert terwijl de Configuration Manager-console geopend is, moet u de Configuration Manager-console opnieuw opstarten nadat de installatie van de VMM-console is voltooid. Anders Configuration Manager zal geen verbinding maken met de VMM-beheerserver om een VHD te uploaden.  
...
###  <a name="BKMK_CreateTS"></a> Takenreeks voor de VHD maken  
 U moet een takenreeks maken die de stappen bevat om de VHD te maken. In de wizard Takenreeks maken, moet u de optie **Een installatiekopie installeren op een virtuele harde schijf** hebben, die de stappen maakt om de VHD te maken. De wizard voegt bijvoorbeeld de volgende vereiste stappen toe: Opnieuw opstarten in Windows PE, schijf formatteren en partitioneren, besturingssysteem en Computer afsluiten van toepassing. U kunt de VHD niet maken in het volledige besturingssysteem. Configuration Manager moet ook wachten tot de virtuele machine is afgesloten voordat het pakket kan worden voltooid. De wizard wacht standaard 5 minuten voordat deze de virtuele machine afsluit. Nadat u de takenreeks maakt, kunt u indien nodig aanvullende stappen toevoegen.  
...
6.  Geef op de pagina **Windows installeren** de volgende opties op en klik op **Volgende**.  
    ...
    -   **Afbeelding**: Als het besturingssysteem meerdere installatiekopieën bevat, voert u de index van de installatiekopie van het besturingssysteem te installeren.  
    ...
    -   Geef op hoe het beheerdersaccount moet worden afgehandeld dat wordt gebruikt wanneer de installatiekopie van het besturingssysteem wordt geïmplementeerd.  
        ...
        -   **Willekeurig wachtwoord genereren voor lokale beheerder en het account uitschakelen op alle ondersteunde platforms (aanbevolen)**: Gebruik deze instelling om de wizard een willekeurig wachtwoord kan maken voor het lokale beheerdersaccount en het account uitschakelen wanneer de installatiekopie van het besturingssysteem wordt geïmplementeerd.  

        -   **Het account inschakelen en geef het lokale beheerderswachtwoord**: Gebruik deze instelling met een specifiek wachtwoord voor het lokale beheerdersaccount op alle computers waarop de installatiekopie van het besturingssysteem wordt geïmplementeerd.  

7.  Geef op de pagina **Netwerk configureren** de volgende opties op en klik op **Volgende**.  

    -   **Lid worden van een werkgroep**: Geef op of de doelcomputer toevoegen aan een werkgroep.  

    -   **Toevoegen aan een domein**: Geef op of de doelcomputer aan een domein toevoegen. Geef in **Domein**de naam op van het domein.  
...
8.  Op de **Configuration Manager installeren** pagina, geeft u de Configuration Manager-clientpakket om te installeren op de doelcomputer en klik vervolgens op **volgende**.  
...
3.  Klik op **Virtuele harde schijf maken** in het tabblad **Start** , in de groep **Maken** om de wizard Virtuele harde schijf maken te starten.  
    ...
    > [!NOTE]  
    >  Hyper-V moet zijn geïnstalleerd op de computer waarop de Configuration Manager-console van waaruit u de VHD's beheert of de **virtuele harde schijf maken** optie is niet ingeschakeld. Zie [Installatievereisten voor Hyper-V](http://technet.microsoft.com/library/cc731898.aspx)voor meer informatie over de vereisten voor Hyper-V.  
...
4.  Stel op de pagina **Algemeen** de volgende instellingen in en klik op **Volgende**.  
    ...
    -   **Pad**: Het pad en de bestandsnaam opgeven waar de wizard het VHD-bestand zal maken.  

         U moet een geldig netwerkpad in de UNC-indeling invoeren. Bijvoorbeeld:  **\\\servername\\< sharename\>\\< bestandsnaam\>.vhd**.  

        > [!WARNING]  
        >  Configuration Manager moet hebben **schrijven** toegang hebben tot het opgegeven pad om de VHD te maken. Configuration Manager kan geen toegang tot het pad, registreert hij de gekoppelde foutmelding in het distmgr.log-bestand op de siteserver.  
...
8.  Controleer de instellingen en klik vervolgens op **Volgende**. De wizard maakt de VHD.  
    ...
    > -   **DeployToVHD.log**: De wizard schrijft informatie in dit logboek tijdens het proces om de VHD te maken. Bekijk dit logboekbestand voor het opvolgen van de voortgang van de wizard voor alle stappen na het maken van de zelfstandige media.  
    >   
 ...
 Configuration Manager maakt de VHD, bevindt zich in de **virtueel hardeschijfstation** knooppunt in de Configuration Manager-console onder de **Besturingssysteemimplementatie** knooppunt in de **softwarebibliotheek** werkruimte.  

> [!NOTE]  
>  Configuration Manager haalt de grootte van de VHD door verbinding te maken met de bronlocatie van de VHD. Als Configuration Manager heeft geen toegang het VHD-bestand tot **0** wordt weergegeven in de **grootte (KB)** kolom voor de VHD.  
...
3.  Klik op **Virtuele harde schijf wijzigen** op het tabblad **Start** in de groep **Virtuele harde schijf** om de wizard Virtuele harde schijf wijzigen te starten.  
    ...
    > [!NOTE]  
    >  Hyper-V moet zijn geïnstalleerd op de computer waarop de Configuration Manager-console van waaruit u de VHD's beheert of de **virtuele harde schijf wijzigen** optie is niet ingeschakeld. Zie [Installatievereisten voor Hyper-V](http://technet.microsoft.com/library/cc731898.aspx)voor meer informatie over de vereisten voor Hyper-V.  

4.  Bevestig op de pagina **Algemeen** de volgende instellingen en klik vervolgens op **Volgende**.  

    -   **Naam**: Geeft een unieke naam voor de VHD.  

    -   **Versie**: Geeft een versienummer voor de VHD. Dit is een optionele instelling.  

    -   **Opmerking**: Hiermee geeft u de beschrijving voor de VHD.  

    -   **Pad**: Hiermee geeft u de naam van het pad en de bestandsnaam op waar het VHD-bestand. U kunt deze instelling niet wijzigen.  
...
8.  Controleer de instellingen en klik vervolgens op **Volgende**. De wizard maakt de gewijzigde VHD.  
    ...
    >   
    >  -   **CreateTSMedia.log**: De wizard schrijft informatie in dit logboek bij het maken van de takenreeksmedia. Bekijk dit logboekbestand om de voortgang van de wizard op te volgen bij het maken van de zelfstandige media.  
    > -   **DeployToVHD.log**: De wizard schrijft informatie in dit logboek tijdens het proces om de VHD te wijzigen. Bekijk dit logboekbestand voor het opvolgen van de voortgang van de wizard voor alle stappen na het maken van de zelfstandige media.  
    >   
...
##  <a name="BKMK_ApplyUpdates"></a> Software-updates toepassen op een VHD  
 Er worden regelmatig nieuwe software-updates uitgebracht die van toepassing zijn op het besturingssysteem in uw VHD. U kunt de toepasselijke software-updates op een VHD toepassen volgens een opgegeven planning. Volgens de planning die u opgeeft, is de software-updates die u selecteert door Configuration Manager van toepassing op de VHD.  
 ...
 Kunt u de **Doorgaan bij fout** instellen voor Configuration Manager om te blijven toepassen van software-updates zelfs wanneer er een fout bij het toepassen van een of meer van de software-updates die u hebt geselecteerd.  
...
4.  Configureer op de pagina **Algemeen** de volgende instellingen en klik daarna op **Volgende**.  
    ...
    -   **VMM-servernaam**: Geef de FQDN van de computer waarop de VMM-beheerserver is geïnstalleerd. The wizard maakt verbinding met de VMM-beheerserver om de bibliotheekshares voor de server te downloaden.  
