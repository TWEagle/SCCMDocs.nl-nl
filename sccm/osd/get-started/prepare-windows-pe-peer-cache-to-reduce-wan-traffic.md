---
title: Voorbereiden van Windows PE-peercache om WAN-verkeer te beperken | Microsoft-documenten
description: De Windows PE-Peercache werkt in de Windows PE voor het ophalen van inhoud van een lokale peer en WAN-verkeer te beperken wanneer er geen lokaal distributiepunt.
ms.custom: na
ms.date: 10/06/2016
...
Wanneer u een nieuw besturingssysteem in System Center Configuration Manager implementeert, kunt computers waarop de takenreeks wordt uitgevoerd Windows PE-Peercache inhoud ophalen van een lokale peer (een peer-cachebron) in plaats van inhoud wordt gedownload van een distributiepunt. Dit helpt het Wide Area Network-verkeer (WAN) te beperken indien er sprake is van meerdere filialen terwijl er geen lokaal distributiepunt bestaat.  
 ...
 Windows PE-Peercache is vergelijkbaar met [Windows BranchCache](http://technet.microsoft.com/library/mt617255\(TechNet.10\).aspx#bkmk_branchcache), maar de functies in de Windows Preinstallation Environment (Windows PE). Als u de takenreeks start vanuit een besturingssysteemcontext, bijvoorbeeld via het Software Center op de client, wordt Windows PE-Peer-Cache niet gebruikt. De volgende termen worden gebruikt om de clients te beschrijven die Windows PE-Peer-Cache gebruiken:  
...
Gebruik de volgende secties voor het beheren van Peer-Cache.
...
-   Pakketten en programma's (als de client gaat door de takenreeks in het volledige besturingssysteem, de opgehaald van de client deze inhoud van een peer-cache-bron als de takenreeks oorspronkelijk is geconfigureerd voor peercache bij het uitvoeren van Windows PE.)  
...
##  <a name="BKMK_PeerCacheWork"></a> Hoe werkt Windows PE-Peer-Cache?  
 Stelt u zich een scenario voor met een filiaal dat geen distributiepunt heeft, maar wel enkele clients heeft waarvoor gebruik van Windows PE-Peer-Cache is ingeschakeld. U implementeert de takenreeks die is geconfigureerd om Peer-Cache te gebruiken op verschillende clients die zijn geconfigureerd om onderdeel te vormen van de Peer-Cache-bron. De eerste client die de takenreeks uitvoert, zendt een verzoek uit voor een peer met de inhoud. Deze peer wordt niet gevonden en de inhoud wordt daarom opgehaald van een distributiepunt via het WAN. De client de installatiekopie van het nieuwe installeert en vervolgens de inhoud opslaan in de clientcache voor Configuration Manager zodat deze als een bron van de cache peer met andere clients werken kan. Wanneer de volgende client de takenreeks uitvoert, wordt er via het subnet een aanvraag verstuurd voor een peer-cachebron. De eerste client reageert dan en maakt de inhoud in de cache beschikbaar.  
...
-   De Configuration Manager-client moet communiceren via de volgende poorten op uw netwerk:  

    -   Poort voor de eerste netwerkuitzending om een peer-cachebron te vinden. Dit is standaard poort 8004.  

    -   Poort voor de inhoud downloaden van een bron van de cache peer (HTTP en HTTPS). Dit is standaard poort 8003.  
...
-   Een peer-cacheclient inhoud kan worden opgehaald uit een andere peer-cacheclient (een bron peer-cache).  Omdat de client is geconfigureerd voor peer-cache, wordt de client een peer-cachebron wanneer deze een takenreeks uitvoert op basis waarvan de inhoud in de cache moet worden bewaard.  
...
1.  Navigeer in de Configuration Manager-console naar **beheer** > **clientinstellingen**, en maak vervolgens een nieuwe **aangepaste Clientapparaatinstellingen** of een bestaande instellingenobject bewerken. U kunt dit ook configureren voor het object **Standaardclientinstellingen** .  
...
-   **SMSTSPeerRequestPort**  
     ...
     Als u niet de standaardpoort die is geconfigureerd in de clientinstellingen (8004) gebruikt, configureert u deze variabele met een aangepaste waarde van de netwerkpoort moet worden gebruikt voor de eerste uitzending.  

-   **SMSTSPreserveContent**  

     Waarde: DE WAARDE TRUE  

     Dit vlaggen die de inhoud in de takenreeks moet worden behouden in de clientcache voor Configuration Manager na de implementatie. Dit is anders dan het gebruik van SMSTSPersisContent die alleen de inhoud voor de duur van de takenreeks, blijft behouden en maakt gebruik van de takenreekscache niet de clientcache voor Configuration Manager.  
...
###  <a name="BKMK_PeerCacheValidate"></a> Het succes van het gebruik van Windows PE-Peer-Cache valideren  
 ...
 Zoek een vergelijkbaar met de volgende vermelding in het logboek waarbij <*Bronservernaam*> identificeert de computer waar de client de inhoud verkregen. Deze computer moet een peer-cachebron zijn, en geen distributiepuntserver. Andere details zijn afhankelijk van uw lokale omgeving en configuraties.  

-   *<! [Logboek [bestand gedownload van http:// < Bronservernaam\>: 8003/SCCM_BranchCache$/SS10000C/sccm?/install.wim c:\\_SMSTaskSequence\Packages\SS10000C\install.wim] logboek]! >< tijd = "14:24:33.329 + 420" datum = "06 26 2015" onderdeel = "ApplyOperatingSystem" context = "" type = "1" thread = "1256" file="downloadcontent.cpp:1626" >*  
