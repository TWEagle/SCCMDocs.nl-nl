---
title: Gebruikersstatus beheren - Configuration Manager | Microsoft-documenten
description: System Center Configuration Manager maakt gebruik van het User State Migration Tool vastleggen en herstellen van gebruikersstatusgegevens in implementatiescenario&quot;s van besturingssysteem.
ms.custom: na
ms.date: 01/23/2017
...
U kunt takenreeksen System Center Configuration Manager vastleggen en herstellen van de gegevens in implementatiescenario's van besturingssysteem waar u de status van de gebruiker van het huidige besturingssysteem behouden. Bijvoorbeeld:  
...
-   Werk implementaties bij waarvan u de gebruikersstatus op dezelfde computer wilt vastleggen en herstellen.  
 ...
 Configuration Manager gebruikt User State Migration Tool (USMT) 10.0 voor het migreren van gegevens van een broncomputer naar een doelcomputer nadat de installatie van het besturingssysteem is voltooid. Zie  [Algemene migratiescenario's](https://technet.microsoft.com/library/mt299169\(v=vs.85\).aspx)voor meer informatie over algemene migratiescenario's voor USMT 10.0.  

 Gebruik de volgende secties bij het vastleggen en herstellen van gebruikersgegevens.


##  <a name="BKMK_StoringUserData"></a> Het opslaan van gebruikersstatusgegevens  
 Wanneer u de gebruikersstatus vastlegt, kunt u de gebruikersstatusgegevens opslaan op de doelcomputer of op een statusmigratiepunt. Om de gebruikersstatus opslaan op een gebruikersstatusmigratiepunt, moet u een Configuration Manager-sitesysteemserver die als host fungeert voor de status van migratie-puntsitesysteemrol gebruiken. U moet, om de gebruikersstatus op de doelcomputer op te slaan, uw takenreeks configureren om de gegevens lokaal op te slaan met behulp van koppelingen.  
...
3.  [Maak een takenreeks voor het vastleggen en herstellen van gebruikersstatus in System Center Configuration Manager](../deploy-use/create-a-task-sequence-to-capture-and-restore-user-state.md). Specifiek, moet u de volgende takenreeksstappen voor het vastleggen van gebruikersgegevens van een computer, de gebruikersdatum op een statusmigratiepunt opslaan en herstellen de gebruikersgegevens op een computer toevoegen:  

    -   [Statusopslag opvragen](../understand/task-sequence-steps.md#BKMK_RequestStateStore) toegang vragen tot een statusmigratiepunt bij het vastleggen van status van een computer of de status is hersteld naar een computer.  

    -   [Gebruikersstatus vastleggen](../understand/task-sequence-steps.md#BKMK_CaptureUserState) vastleggen en opslaan van de gegevens op het statusmigratiepunt.  

    -   [Gebruikersstatus herstellen](../understand/task-sequence-steps.md#BKMK_RestoreUserState) te herstellen van de gebruikersstatus op de doelcomputer met ophalen van de gegevens van een gebruikersstatusmigratiepunt.  
...
-   [Maak een takenreeks voor het vastleggen en herstellen van gebruikersstatus](../deploy-use/create-a-task-sequence-to-capture-and-restore-user-state.md). U moet met name de volgende takenreeksstappen toevoegen om de gebruikersgegevens van een computer vast te leggen en de gebruikersgegevens met vaste koppelingen op een computer terug te zetten:  
    ...
    -   [Gebruikersstatus vastleggen](../understand/task-sequence-steps.md#BKMK_CaptureUserState) vastleggen en opslaan van de gegevens naar een lokale map met behulp van vaste koppelingen.  

    -   [Gebruikersstatus herstellen](../understand/task-sequence-steps.md#BKMK_RestoreUserState) de status van de gebruiker op de doelcomputer herstellen met de gegevens met vaste koppelingen worden opgehaald.  
...
-   Of het statusmigratiepunt alleen moet reageren op aanvragen om gebruikersstatusgegevens terug te zetten. Wanneer u deze optie inschakelt, kunt het statusmigratiepunt niet gebruiken om de gebruikersstatusgegevens op te slaan.  
 ...
 Zie voor meer informatie over het statusmigratiepunt en de stappen om de configuratie [Statusmigratiepunt](prepare-site-system-roles-for-operating-system-deployments.md#BKMK_StateMigrationPoints).  

##  <a name="BKMK_ComputerAssociation"></a> Create a computer association  
 Maak een computerkoppeling om een relatie te definiëren tussen een broncomputer en een doelcomputer wanneer u een besturingssysteem op nieuwe hardware installeert en gebruikersgegevensinstellingen wilt vastleggen en terugzetten. De broncomputer is een bestaande computer die Configuration Manager beheert. Wanneer u het nieuwe besturingssysteem implementeert op de doelcomputer, bevat de broncomputer de gebruikersstatus die is gemigreerd naar de doelcomputer.  

> [!NOTE]  
>  Maak een computerkoppeling tussen computers die zich in een bovenliggende site van Configuration Manager met computers die zich in een onderliggende site wordt niet ondersteund. Computerkoppelingen zijn sitespecifiek en worden niet gerepliceerd.  
