...
2.  Bevat informatie over de objecten die u kunt selecteren voor het importeren en informatie over waarom een object kan niet worden geïmporteerd.
...
> [!Note]    
> Het importeren van implementaties is alleen nuttig voor andere objecttypen die u wilt importeren. Bijvoorbeeld, als u VPN-profielen en implementaties importeert, wordt het alleen mogelijk voor het importeren van de implementaties voor de VPN-profielen die u selecteert. Implementaties in Intune worden toewijzingen genoemd. Als u importeren van een implementatie voor een eerder geïmporteerd object wilt, moet u voor het importeren van dat object de toewijzing againor handmatig maken in Intune in Azure. Bijvoorbeeld, als u VPN-profielen en niet-implementaties importeert, hebt u het hulpprogramma opnieuw uitvoeren en VPN-profielen en -implementaties selecteren of handmatig de toewijzing te maken in Intune in Azure.  Als u het hulpprogramma opnieuw uit, worden dubbele objecten mogelijk gemaakt dat u later in Intune in Azure kunt verwijderen.  
...
- Er zijn een aantal profielen die afhankelijk van andere objecten zijn. Als u wilt importeren van een profiel dat is afhankelijk van een ander object, zoals een e-mailprofiel die afhankelijk zijn van een certificaat, moet u beide objecten op hetzelfde moment importeren.
...
- U moet het hulpprogramma uitvoeren vanaf een computer die toegang tot de SMS-Provider heeft (Configuration Manager om gegevens te verzamelen) en het Internet (om objecten te importeren naar Intune).
...
1. Ga naar de [Intune gegevens importfunctie van GitHub](https://go.microsoft.com/fwlink/?linkid=858194) pagina.
2. Klik op **klonen of downloaden**, klikt u op **ZIP downloaden**, en sla het gecomprimeerde ZIP-bestand. 
3. Pak de inhoud van het ZIP-bestand.
...
3. Geef de volgende informatie over uw site en de objecten op de site die u wilt importeren. 
...
5.  Klik op **volgende** om te controleren van de objecten die worden verzameld. Pagina voor het selecteren van een item is beschikbaar voor elk objecttype die worden verzameld. 
    ...
    > [!Tip]     
    > Op elke pagina van de selectie item kunt u een filter waarmee u de objecten die u wilt importeren. Echter, dient u het volgende:
    > - Wanneer u op de pagina van een item selecteren en de weergave is gefilterd, selecteer alle selectievakjes alleen van toepassing op de items weergegeven. Verborgen objecten vanwege een filter zijn niet opgenomen wanneer u de selectievakjes uit.
    > - Objecten worden altijd gegroepeerd onder hun bovenliggende item, zelfs wanneer u sorteren of filteren van de objecten.
...
8.  Selecteer de objecten die u wilt importeren op elke pagina item selecteren. De volgende kolommen worden weergegeven:
    ...
    - **Opmerkingen bij de**: Bevat informatie over waarom een object kan niet worden geïmporteerd.
...
### <a name="phase-3-import-selected-object-to-intune"></a>Fase 3: Importeren van het geselecteerde object naar Intune
In de fase 3, u aanmelden bij Intune en de geselecteerde objecten importeren. 
10. Klik op **volgende**, en klik vervolgens op **aanmelden bij Intune** zich aanmeldt bij de Intune-tenant voor het importeren van gegevens. U moet zich aanmelden met een globale beheerder of de Intune-beheerder. Nadat u zich aanmeldt, wordt het importproces gestart.
